...
export const getUserFromToken = (token) => {
  return function(dispatch) {
    ...
    const request = axios({
      method: 'get',
      url: `${API_ROOT}/auth/getUserFromToken`,
      headers: {
        'Authorization': token
      }
    });
...
export const loginRequest = (data) => {
  return function(dispatch) {
    dispatch(setLoadingTrue());
    const request = axios.post(`${API_ROOT}/auth/login`, {email: data.email, password: data.password});
    request.then( (response) => {
...
export const registerRequest = (data) => {
  return function(dispatch) {
    dispatch(setLoadingTrue());
    const request = axios.post(`${API_ROOT}/auth/register` , {
      email: data.email,
